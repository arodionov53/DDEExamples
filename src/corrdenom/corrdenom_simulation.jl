# Discrete-time corrected-denominator budget pacing simulation
# Adapts the PID simulation framework to use a corrected-denominator controller

include("corrdenom_pacer.jl")
include("corrdenom_simulation_use_cases.jl")

# ── Main Entry Points ────────────────────────────────────────────────────────

function run_single_corrdenom_scenario!(uc::SimulationUseCase; plot::Bool=true, verbose::Bool=false, τ::Float64=5.0, index::Int=0)
    # Initialize
    tick_with!(uc, 0.0)

    pacer = CorrDenomPacer(uc.state.current_time; τ)
    ...
    while should_tick(uc, iterations)
        ...
        # Build corrected-denominator input and calculate
        result = calculate_corrdenom_mode!(pacer;
    ...
    if plot && !isempty(plot_data)
        plot_name = "CorrDenomPacer_$(index)_$(uc.name)"
        plot_simulation(plot_name, plot_data, uc.metadata.start_time, uc.metadata.end_time;
            output_dir="src/corrdenom/plots")
    end

    return (hard_violations, soft_violations, iterations)
end

function run_corrdenom_simulation(; scenario::Union{Nothing,String}=nothing, plot::Bool=true, verbose::Bool=false, τ::Float64=5.0)
    ...
    for (i, uc) in enumerate(use_cases)
        ...
        hv, sv, iters = run_single_corrdenom_scenario!(uc; plot, verbose, τ, index=i-1)
