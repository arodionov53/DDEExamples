# Discrete-time Artstein predictor budget pacing simulation
# Uses integral of spend rate over [t-τ, t] instead of point-difference

include("artstein_pacer.jl")
include("artstein_simulation_use_cases.jl")

# ── Main Entry Points ────────────────────────────────────────────────────────

function run_single_artstein_scenario!(uc::SimulationUseCase; plot::Bool=true, verbose::Bool=false, τ::Float64=5.0, index::Int=0)
    # Initialize
    tick_with!(uc, 0.0)

    pacer = ArtsteinPacer(uc.state.current_time; τ)
    ...
    while should_tick(uc, iterations)
        ...
        # Build Artstein input and calculate
        result = calculate_artstein_mode!(pacer;
    ...
    if plot && !isempty(plot_data)
        plot_name = "ArtsteinPacer_$(index)_$(uc.name)"
        plot_simulation(plot_name, plot_data, uc.metadata.start_time, uc.metadata.end_time;
            output_dir="src/artstein/plots")
    end

    return (hard_violations, soft_violations, iterations)
end

function run_artstein_simulation(; scenario::Union{Nothing,String}=nothing, plot::Bool=true, verbose::Bool=false, τ::Float64=5.0)
    ...
    for (i, uc) in enumerate(use_cases)
        ...
        hv, sv, iters = run_single_artstein_scenario!(uc; plot, verbose, τ, index=i-1)
